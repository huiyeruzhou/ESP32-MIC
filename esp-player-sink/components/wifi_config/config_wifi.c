...
static void example_set_static_ip(esp_netif_t *netif) {
  ...
  ESP_LOGD(TAG, "Success to set static ip: %s, netmask: %s, gw: %s", ip_ip,
           ip_netmask, ip_gw);
  ESP_ERROR_CHECK(
      example_set_dns_server(netif, ipaddr_addr(ip_gw), ESP_NETIF_DNS_MAIN));
...
static void event_handler(void *arg, esp_event_base_t event_base,
  ...
  if (event_base == WIFI_EVENT && event_id == WIFI_EVENT_STA_START) {
    esp_wifi_connect();
  } else if (ip_static && event_base == WIFI_EVENT &&
    ...
    int len = snprintf(ip_connected, sizeof(ip_connected), IPSTR,
...
const char *get_ip_connected() { return is_ip_connected ? ip_connected : ""; }
