...
esp_err_t api_AbilityRequest_handler(httpd_req_t *req) {
  ...
  if (buf_len > 1) {
    ...
    if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
      ...
      if (httpd_query_key_value(buf, "abilityName", param, sizeof(param)) ==
          ESP_OK) {
      ...
      if (status == 200 &&
          httpd_query_key_value(buf, "port", param, sizeof(param)) == ESP_OK) {
      ...
      if (status == 200 &&
          httpd_query_key_value(buf, "cmd", param, sizeof(param)) == ESP_OK) {
        ...
        for (i = 0; i < 4; i++) {
          if (strcmp(param, cmd[i]) == 0) {
            if (!speakerContext->check_cmd_legal(i)) {
              ESP_LOGE(TAG, "cannot perform `%s` on `%s` status", param,
                       speakerContext->getStatusString());
              resp_body += "cannot perform `" + std::string(param) + "` on `" +
                           std::string(speakerContext->getStatusString()) +
                           "` status\n";
              status = 404;
            } else {
      ...
      if (status == 200 && httpd_query_key_value(buf, "connectIP", param,
                                                 sizeof(param)) == ESP_OK) {
        ESP_LOGI(TAG, "Found URL query parameter => connectIP=%s", param);
      }
      if (status == 200 && httpd_query_key_value(buf, "connectPort", param,
                                                 sizeof(param)) == ESP_OK) {
