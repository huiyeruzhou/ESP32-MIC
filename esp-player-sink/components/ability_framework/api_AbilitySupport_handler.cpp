...
const static char *TAG = "API_SUPPORT_HANDLER";

/* An HTTP GET handler */
esp_err_t api_AbilitySupport_handler(httpd_req_t *req) {
  char *buf;
  size_t buf_len;
  ...
  /* 设置响应体*/
  char *resp_str = (char *)((reinterpret_cast<AbilityContext *>(req->user_ctx))
                                ->getAbilitySupport());
...
extern const httpd_uri_t api_AbilitySupport = {
    .uri = "/api/AbilitySupport",
    .method = HTTP_GET,
    .handler = api_AbilitySupport_handler,
